...
type CustomAsset = { id: string; icon: string; name: string; sub: string; value: number; change: number | null; category: string };
...
export default function AssetsScreen() {
  ...
  const [newValue, setNewValue] = useState(''); const [newChange, setNewChange] = useState('');
  const [newIcon, setNewIcon] = useState('🏠'); const [newCat, setNewCat] = useState('property');
  const [newHasChange, setNewHasChange] = useState(false);
  const [vMake, setVMake] = useState(''); const [vModel, setVModel] = useState('');
  const [vYear, setVYear] = useState(''); const [vValue, setVValue] = useState(''); const [vIcon, setVIcon] = useState('🚗');
  const [pAddress, setPAddress] = useState(''); const [pType, setPType] = useState('Residential');
  const [pValue, setPValue] = useState(''); const [pMortgage, setPMortgage] = useState('');
  ...
  const toggle = (label: string) => setOpenCats(prev => prev.includes(label) ? prev.filter(l => l !== label) : [...prev, label]);
  ...
  const addAsset = () => {
    ...
    if (addMode === 'vehicle') {
      ...
      asset = { id: Date.now().toString(), icon: '🏠', name: pAddress, sub: `${pType} · ${t('equity')}: ${formatAmount(eq)}`, value: parseFloat(pValue), change: null, category: 'property' };
          ...
          return (
            <View key={section.label} style={{ backgroundColor: c.card, borderRadius: 20, marginBottom: 12, borderWidth: 1, borderColor: c.border, overflow: 'hidden' }}>
              ...
              {isOpen && section.items.map((item: any, i: number) => (
                <TouchableOpacity key={i} onLongPress={() => item.id && setCustomAssets(prev => prev.filter(a => a.id !== item.id))}
            ...
            <View style={{ padding: 24 }}>
              {addMode === 'vehicle' && (
                <>
                  ...
                  {[
                    { label: t('make'), val: vMake, set: setVMake, ph: 'e.g. Ford' },
                    { label: t('model'), val: vModel, set: setVModel, ph: 'e.g. Focus' },
                    { label: t('year'), val: vYear, set: setVYear, ph: 'e.g. 2021' },
                    { label: `${t('estValue')} (${currencySymbol})`, val: vValue, set: setVValue, ph: 'e.g. 12500' },
                  ].map((f, i) => (
                    <View key={i} style={{ marginBottom: 12 }}>
                      <Text style={{ color: c.muted, fontSize: 12, fontWeight: '600', marginBottom: 6 }}>{f.label}</Text>
                      <TextInput style={{ backgroundColor: c.card2, borderRadius: 12, padding: 14, color: c.text, fontSize: 15, borderWidth: 1, borderColor: c.border }} placeholder={f.ph} placeholderTextColor={c.muted} value={f.val} onChangeText={f.set} keyboardType={f.label.includes(currencySymbol) || f.label === t('year') ? 'decimal-pad' : 'default'} />
                    </View>
                  ))}
                </>
              )}
              {addMode === 'property' && (
                <>
                  ...
                  {[
                    { label: t('address'), val: pAddress, set: setPAddress, ph: 'e.g. 12 Oak Street' },
                    { label: `${t('estMarketValue')} (${currencySymbol})`, val: pValue, set: setPValue, ph: 'e.g. 250000' },
                    { label: `${t('outstandingMortgage')} (${currencySymbol})`, val: pMortgage, set: setPMortgage, ph: 'e.g. 150000' },
                  ].map((f, i) => (
                    <View key={i} style={{ marginBottom: 12 }}>
                      <Text style={{ color: c.muted, fontSize: 12, fontWeight: '600', marginBottom: 6 }}>{f.label}</Text>
                      <TextInput style={{ backgroundColor: c.card2, borderRadius: 12, padding: 14, color: c.text, fontSize: 15, borderWidth: 1, borderColor: c.border }} placeholder={f.ph} placeholderTextColor={c.muted} value={f.val} onChangeText={f.set} keyboardType={f.label.includes(currencySymbol) ? 'decimal-pad' : 'default'} />
                    </View>
                  ))}
                  {pValue && pMortgage ? (
                    <View style={{ backgroundColor: c.card2, borderRadius: 12, padding: 12, marginBottom: 12, borderWidth: 1, borderColor: c.border }}>
                      <Text style={{ color: c.muted, fontSize: 12 }}>{t('equity')}: <Text style={{ color: '#00D4AA', fontWeight: '700' }}>{formatAmount(parseFloat(pValue) - parseFloat(pMortgage))}</Text></Text>
                    </View>
                  ) : null}
                </>
              )}
