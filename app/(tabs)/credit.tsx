...
const tips = [
  { icon: '📅', text: 'Pay all bills on time — payment history is the biggest factor' },
  { icon: '💳', text: 'Keep credit card usage below 30% of your total limit' },
  { icon: '🔍', text: 'Avoid applying for multiple credit products in a short period' },
  { icon: '📋', text: 'Register on the electoral roll at your current address' },
  { icon: '⏳', text: 'Keep old accounts open — longer credit history helps your score' },
];
        ...
        <View style={{ backgroundColor: c.card, borderRadius: 20, padding: 18, marginBottom: 40, borderWidth: 1, borderColor: c.border }}>
          {tips.map((tip, i) => (
            <View key={i} style={{ flexDirection: 'row', gap: 12, paddingVertical: 10, borderBottomWidth: i < tips.length - 1 ? 1 : 0, borderBottomColor: c.border, alignItems: 'flex-start' }}>
              ...
              <Text style={{ color: c.muted, fontSize: 13, flex: 1, lineHeight: 18 }}>{tip.text}</Text>
            </View>
          ))}
        </View>
      </ScrollView>
