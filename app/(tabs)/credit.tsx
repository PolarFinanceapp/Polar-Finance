...
const tips = [
  { icon: '📅', tip: 'Pay all bills on time — payment history is the biggest factor' },
  { icon: '💳', tip: 'Keep credit card usage below 30% of your total limit' },
  { icon: '🔍', tip: 'Avoid applying for multiple credit products in a short period' },
  { icon: '📋', tip: 'Register on the electoral roll at your current address' },
  { icon: '⏳', tip: 'Keep old accounts open — longer credit history helps your score' },
];
...
export default function CreditScreen() {
  ...
  return (
    <ScrollView style={{ flex: 1, backgroundColor: c.dark, paddingHorizontal: 20 }} showsVerticalScrollIndicator={false}>
      <Text style={{ color: c.text, fontSize: 26, fontWeight: '900', marginTop: 60, marginBottom: 6 }}>Credit Score 💳</Text>
      <Text style={{ color: c.muted, fontSize: 13, marginBottom: 24 }}>Check your free credit score with a trusted UK provider</Text>
      ...
      <View style={{ backgroundColor: c.card2, borderRadius: 16, padding: 16, marginBottom: 24, borderWidth: 1, borderColor: c.border, flexDirection: 'row', gap: 12, alignItems: 'flex-start' }}>
        ...
        <View style={{ flex: 1 }}>
          <Text style={{ color: c.text, fontSize: 13, fontWeight: '700', marginBottom: 4 }}>Why we don't show your score directly</Text>
          <Text style={{ color: c.muted, fontSize: 12, lineHeight: 18 }}>
            Credit scores are held by Experian, Equifax and TransUnion. Accessing them directly requires FCA authorisation. We recommend these trusted free services instead.
          </Text>
        </View>
      </View>

      {/* Services */}
      <Text style={{ color: c.muted, fontSize: 12, fontWeight: '700', letterSpacing: .8, textTransform: 'uppercase', marginBottom: 12 }}>Free Credit Score Services</Text>
      {services.map((s, i) => (
        <TouchableOpacity
          key={i}
          style={{ backgroundColor: c.card, borderRadius: 20, padding: 18, marginBottom: 12, borderWidth: 1, borderColor: c.border }}
          ...
          <View style={{ backgroundColor: s.color + '18', borderRadius: 10, padding: 10, marginTop: 12, flexDirection: 'row', alignItems: 'center', gap: 8 }}>
            <Text style={{ color: s.color, fontSize: 12, fontWeight: '700', flex: 1 }}>Check your score free on {s.name} →</Text>
          </View>
        </TouchableOpacity>
      ))}

      {/* Tips */}
      <Text style={{ color: c.muted, fontSize: 12, fontWeight: '700', letterSpacing: .8, textTransform: 'uppercase', marginBottom: 12, marginTop: 8 }}>Tips to Improve Your Score</Text>
      <View style={{ backgroundColor: c.card, borderRadius: 20, padding: 18, marginBottom: 40, borderWidth: 1, borderColor: c.border }}>
        {tips.map((t, i) => (
          <View key={i} style={{ flexDirection: 'row', gap: 12, paddingVertical: 10, borderBottomWidth: i < tips.length - 1 ? 1 : 0, borderBottomColor: c.border, alignItems: 'flex-start' }}>
            ...
            <Text style={{ color: c.muted, fontSize: 13, flex: 1, lineHeight: 18 }}>{t.tip}</Text>
          </View>
        ))}
      </View>
