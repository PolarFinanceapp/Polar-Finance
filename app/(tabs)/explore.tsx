  ...
  const fetchData = async () => {
    try {
      ...
      if (!session?.access_token) {
        setError('Not authenticated - please log out and back in.');
        setLoading(false);
        setRefreshing(false);
        return;
      }
      ...
      console.log('[Markets] status:', res.status, 'body:', text);
    ...
    <ScrollView
      ...
      <View style={{ flexDirection: 'row', backgroundColor: c.card, borderRadius: 50, padding: 4, marginBottom: 20, borderWidth: 1, borderColor: c.border }}>
        {(['stocks', 'commodities', 'crypto'] as const).map(tb => (
          <TouchableOpacity
            key={tb}
            style={{ flex: 1, paddingVertical: 8, borderRadius: 50, alignItems: 'center', backgroundColor: tab === tb ? c.accent : 'transparent' }}
            onPress={() => setTab(tb)}>
      ...
      {loading ? (
        <View style={{ alignItems: 'center', padding: 60 }}>
          ...
          <Text style={{ color: c.muted, marginTop: 8, fontSize: 11, textAlign: 'center', opacity: 0.6 }}>
            First load takes up to 20 seconds
          </Text>
        </View>
          ...
          {tab === 'crypto' && (
            crypto.length === 0
              ? <View style={{ alignItems: 'center', padding: 40 }}>
                ...
                <Text style={{ color: c.muted, fontSize: 14, textAlign: 'center' }}>No crypto data available.</Text>
              </View>
              : crypto.map((item, i) => (
          ...
          {tab !== 'crypto' && (() => {
            ...
            if (items.length === 0) {
              return (
                <View style={{ alignItems: 'center', padding: 40 }}>
                  <Text style={{ fontSize: 40, marginBottom: 12 }}>📊</Text>
                  <Text style={{ color: c.muted, fontSize: 14, textAlign: 'center' }}>No {tab} data available.</Text>
                </View>
              );
            }
