...
const sigLabel = (s: string) => s === 'buy' ? 'BUY' : s === 'strong_buy' ? 'STRONG BUY' : s === 'sell' ? 'SELL' : s === 'strong_sell' ? 'STRONG SELL' : 'NEUTRAL';
...
export default function ExploreScreen() {
  ...
  // Convert GBP market prices to user's currency
  const fmtPrice = (gbpPrice: number) => formatAmount(convertRaw(gbpPrice));
  const fmtCap = (gbpCap: number) => `${currencySymbol}${(convertRaw(gbpCap) / 1e9).toFixed(1)}B`;
