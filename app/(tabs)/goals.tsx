...
export default function GoalsScreen() {
  ...
  const setGoals = async (data: Goal[]) => {
    setGoalsState(data);
  ...
  const openEditGoal = (g: Goal) => {
    setEditGoal(g); setEditName(g.name); setEditTarget(g.target.toString());
  ...
  const handleSaveEdit = () => {
    ...
    setGoals(goals.map(g => g.id === editGoal.id
      ? { ...g, name: editName, target: parseFloat(editTarget), saved: parseFloat(editSaved) || 0, icon: editIcon, color: editColor }
      : g
    ));
    setEditGoal(null);
  };
  ...
  const addToSaved = (id: string, amount: number) => {
    setGoals(goals.map(g => g.id === id
      ? { ...g, saved: Math.min(Math.max(0, g.saved + amount), g.target) }
      : g
    ));
  };
  ...
  const FormFields = ({ name, setName, target, setTarget, saved, setSaved, icon, setIcon, color, setColor, onSubmit, onCancel, submitLabel }: any) => (
    <>
      ...
      <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 8, marginBottom: 12 }}>
        {GOAL_ICONS.map(ic => (
          <TouchableOpacity key={ic} style={{ width: 42, height: 42, borderRadius: 10, backgroundColor: c.card2, justifyContent: 'center', alignItems: 'center', borderWidth: 1.5, borderColor: icon === ic ? c.accent : 'transparent' }} onPress={() => setIcon(ic)}>
      ...
      <View style={{ flexDirection: 'row', gap: 10, marginBottom: 20 }}>
        {GOAL_COLORS.map(col => (
          <TouchableOpacity key={col} style={{ width: 32, height: 32, borderRadius: 16, backgroundColor: col, borderWidth: 2.5, borderColor: color === col ? '#fff' : 'transparent' }} onPress={() => setColor(col)} />
        ))}
      </View>
  ...
  return (
    <ScrollView style={{ flex: 1, backgroundColor: c.dark, paddingHorizontal: 20 }} showsVerticalScrollIndicator={false}>
          ...
          <View key={g.id} style={{ backgroundColor: c.card, borderRadius: 20, padding: 16, marginBottom: 14, borderWidth: 1, borderColor: c.border }}>
            <View style={{ flexDirection: 'row', alignItems: 'center', marginBottom: 12 }}>
              ...
              <View style={{ alignItems: 'flex-end', gap: 4 }}>
                ...
                <View style={{ flexDirection: 'row', gap: 10 }}>
                  <TouchableOpacity onPress={() => openEditGoal(g)}>
      ...
      {canAddGoal ? (
        <TouchableOpacity style={{ backgroundColor: c.card, borderRadius: 16, padding: 16, alignItems: 'center', borderWidth: 1, borderColor: c.accent + '4D', marginBottom: 40, borderStyle: 'dashed' }} onPress={() => setAddGoalVisible(true)}>
