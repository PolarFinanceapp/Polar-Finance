          ...
          <>
            ...
            <View style={{ flexDirection: 'row', gap: 10, marginBottom: 16 }}>
              {[
                ...
                { label: isOverspent ? 'Overspent' : t('saved'), val: Math.abs(totalSaved), color: isOverspent ? '#FF6B6B' : '#00D4AA', border: isOverspent ? 'rgba(255,107,107,0.3)' : 'rgba(0,212,170,0.3)' },
              ].map((item, i) => (
            ...
            <View style={{ backgroundColor: c.card, borderRadius: 20, padding: 18, marginBottom: 16, borderWidth: 1, borderColor: c.border }}>
              <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
                ...
                <Text style={{ color: over ? '#FF6B6B' : '#00D4AA', fontSize: 11, fontWeight: '600' }}>
                  {totalIncome === 0 ? (totalExpense > 0 ? '⚠️ No income' : '—') : over ? '⚠️ Over budget' : `${budgetPct}% spent`}
                </Text>
              </View>
