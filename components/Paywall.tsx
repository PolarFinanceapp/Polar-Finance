...
export default function Paywall({ visible, onClose, required = false }: Props) {
  ...
  const PLANS: { key: Plan; name: string; emoji: string; color: string; badge: string | null; gbpPrice: number; features: string[] }[] = [
    ...
    {
      key: 'premium', name: t('premiumPlan'), emoji: '👑', color: '#FFD700', badge: '⭐ Best Value', gbpPrice: 7.99,
      features: [
  ...
  const handlePromo = async () => {
    const sanitised = sanitisePromoCode(promoCode);
    if (!sanitised) { setPromoError('Enter a promo code.'); return; }

    // Rate limit: 5 promo attempts per 15 min, 30 min lock
    const rl = await checkRateLimit('promo', 5, 15 * 60 * 1000, 30 * 60 * 1000);
    if (!rl.allowed) {
      setPromoError(`Too many attempts. Try again in ${formatRetryAfter(rl.retryAfterSeconds)}.`);
      return;
    }

    setPromoLoading(true);

    // Small artificial delay to further prevent brute-force timing
    await new Promise(r => setTimeout(r, 300));

    // Timing-safe comparison against all known codes
    let matched: Plan | null = null;
    for (const [code, planKey] of Object.entries(PROMO_CODES)) {
      if (timingSafeEqual(sanitised, code)) { matched = planKey; break; }
    }
    ...
    <Modal visible={visible} animationType="slide" presentationStyle="pageSheet">
      <View style={{ flex: 1, backgroundColor: c.dark }}>
        ...
        <View style={{ paddingHorizontal: 20, paddingTop: 56, paddingBottom: 16, flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }}>
          <View>
            <Text style={{ color: c.text, fontSize: 26, fontWeight: '900' }}>
              {required ? t('chooseAPlan') || 'Choose Your Plan' : t('upgradePolar')}
            </Text>
            <Text style={{ color: c.muted, fontSize: 13, marginTop: 4 }}>
              {required ? t('trialExpired') : t('unlockFullExperience')}
            </Text>
          </View>
          {!required && (
        ...
        <ScrollView style={{ flex: 1, paddingHorizontal: 20 }} showsVerticalScrollIndicator={false}>
          ...
          {(plan === 'pro' || plan === 'premium') && (
            <View style={{ backgroundColor: '#00D4AA22', borderRadius: 14, padding: 14, marginBottom: 16, borderWidth: 1, borderColor: '#00D4AA44', flexDirection: 'row', alignItems: 'center', gap: 10 }}>
              ...
              <Text style={{ color: '#00D4AA', fontSize: 14, fontWeight: '700', flex: 1 }}>
                {plan === 'pro' ? t('proPlan') : t('premiumPlan')} active
              </Text>
            ...
            return (
              <TouchableOpacity key={p.key} disabled={isCur} onPress={() => setSelected(p.key)}
                ...
                <View style={{ flexDirection: 'row', alignItems: 'center', gap: 12, marginBottom: 14 }}>
                  ...
                  {isCur && (
                    <View style={{ backgroundColor: '#00D4AA22', borderRadius: 50, paddingHorizontal: 10, paddingVertical: 4 }}>
                      <Text style={{ color: '#00D4AA', fontSize: 11, fontWeight: '700' }}>Current</Text>
                    </View>
                  )}
                </View>
                {p.features.map((f, i) => (
          ...
          <Text style={{ color: c.muted, fontSize: 11, textAlign: 'center', lineHeight: 16, marginTop: 8, marginBottom: 16 }}>
            {t('subscriptionTerms')}
          </Text>
        </ScrollView>
        ...
        <View style={{ paddingHorizontal: 20, paddingBottom: 36, paddingTop: 12, backgroundColor: c.dark, borderTopWidth: 1, borderTopColor: c.border }}>
          <TouchableOpacity disabled={!selected} onPress={handlePurchase}
            ...
            <Text style={{ color: '#fff', fontSize: 17, fontWeight: '900' }}>
              {selected
                ? `${t('subscribeTo')} ${PLANS.find(p => p.key === selected)?.name}`
                : t('selectPlanAbove')}
            </Text>
          </TouchableOpacity>
            ...
            <View style={{ marginTop: 14, backgroundColor: c.card, borderRadius: 16, padding: 16, borderWidth: 1, borderColor: c.accent + '44' }}>
              ...
              <View style={{ flexDirection: 'row', gap: 10 }}>
                <TextInput
                  ...
                  placeholder="e.g. POLAR-PRO" placeholderTextColor={c.muted}
                  value={promoCode}
                  onChangeText={v => { setPromoCode(sanitisePromoCode(v)); setPromoError(''); }}
                  autoCapitalize="characters" autoCorrect={false}
                  maxLength={MAX_LENGTHS.promoCode}
                />
                <TouchableOpacity onPress={handlePromo}
                  disabled={!promoCode.trim() || promoLoading}
                  style={{ backgroundColor: c.accent, borderRadius: 12, paddingHorizontal: 20, justifyContent: 'center', opacity: promoCode.trim() && !promoLoading ? 1 : 0.4 }}>
